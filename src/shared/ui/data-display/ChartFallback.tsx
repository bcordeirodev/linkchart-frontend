...
export function ChartFallback({ height = 350, title = 'Gráfico', data = [], series = [], error }: ChartFallbackProps) {
				...
				<Box sx={{ mt: 3, width: '100%', maxWidth: 300 }}>
					...
					<Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
						{series.map((serie, index) => (
							...
							>
								<Typography variant="body2">{serie.name || `Série ${index + 1}`}</Typography>
								<Typography
									variant="body2"
									color="text.secondary"
								>
