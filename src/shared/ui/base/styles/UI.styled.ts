...
export const EmptyStateContainer = styled(Paper, {
	...
	return {
		...
		backgroundColor: theme.palette.mode === 'dark' ? `${currentColor}08` : `${currentColor}04`,
		...
		'&:hover': {
			backgroundColor: theme.palette.mode === 'dark' ? `${currentColor}12` : `${currentColor}08`,
...
export const StyledEnhancedPaper = styled(Paper, {
	...
	const variantStyles = {
		...
		glass: {
			borderRadius: theme.spacing(1.5),
			background:
				theme.palette.mode === 'dark' ? alpha(theme.palette.background.paper, 0.8) : 'rgba(255, 255, 255, 0.9)',
