...
export interface HeatmapPoint {
	...
	last_click?: string;
	/** Timestamp do primeiro clique (opcional) */
	first_click?: string;
	/** Visitantes únicos nesta localização (opcional) */
	unique_visitors?: number;
	/** Total de links ativos nesta localização (opcional) */
	total_links?: number;
	/** Dias com atividade nesta localização (opcional) */
	active_days?: number;
	/** Horário médio dos cliques (0-23) (opcional) */
	avg_hour?: number;
	/** Percentual de cliques em fins de semana (opcional) */
	weekend_percentage?: number;
	/** Média de cliques por dia (opcional) */
	clicks_per_day?: number;
	/** Taxa de retenção de visitantes (%) (opcional) */
	visitor_retention?: number;
	/** Horário de pico (0-23) (opcional) */
	peak_hour?: number;
	/** Densidade de localização (proximidade geográfica) (opcional) */
