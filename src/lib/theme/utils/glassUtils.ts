 ...
 */
export const createGlassEffect = (theme: Theme, config: GlassConfig = {}): SxProps => {
	const { intensity = 'medium', tint, border = true, shadow = true, blur } = config;
	...
	if (border) {
		glassStyles.border = `1px solid ${alpha(isDark ? '#ffffff' : '#000000', config_values.borderOpacity)}`;
	}
 ...
 */
export const createGlassCard = (theme: Theme, variant: 'primary' | 'secondary' | 'neutral' = 'neutral'): SxProps => {
	const isDark = theme.palette.mode === 'dark';
 ...
 */
export const createGlassButton = (theme: Theme, variant: 'primary' | 'secondary' = 'primary'): SxProps => {
 ...
 */
export const applyGlassEffect = (existingSx: SxProps, theme: Theme, config: GlassConfig = {}): SxProps => {
