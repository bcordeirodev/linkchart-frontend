 ...
 */
export const getThemeColor = (theme: Theme, config: ColorConfig | ColorVariant): string => {
...
export const getBackgroundColors = (theme: Theme) => {
	...
	return {
		...
		elevated: isDark ? alpha(theme.palette.background.paper, 0.8) : theme.palette.background.paper,
		glass: isDark ? alpha(theme.palette.background.paper, 0.8) : alpha('#ffffff', 0.9),
		overlay: isDark ? alpha('#000000', 0.7) : alpha('#000000', 0.5)
	};
};
