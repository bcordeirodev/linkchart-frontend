...
export const CLIENT_MODULE_CONFIG = {
	...
	REQUIRED_CLIENT_COMPONENTS: ['pages', 'layouts', 'providers', 'forms', 'interactive-components'],
	...
	SERVER_COMPONENTS: ['static-content', 'metadata-generators', 'data-fetchers']
};
...
export const FILE_STRUCTURE_RULES = {
	...
	REQUIRED_MODULE_STRUCTURE: ['page.tsx', 'Header.tsx', 'Metrics.tsx', 'Tabs.tsx']
};
...
export function validateComponentStructure(filePath: string, content: string) {
	...
	const hasClientHooks = CLIENT_MODULE_CONFIG.CLIENT_HOOKS.some((hook) => content.includes(hook));
...
export function optimizeImports(content: string): string {
	...
	optimized = optimized.replace(/import\s*{\s*([^}]+)\s*}\s*from\s*['"]@mui\/material['"]/g, (match, imports) => {
		const importList = imports.split(',').map((imp: string) => imp.trim());
		return importList.map((imp: string) => `import { ${imp} } from '@mui/material/${imp}';`).join('\n');
	});
