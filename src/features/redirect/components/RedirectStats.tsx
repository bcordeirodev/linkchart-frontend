...
export default function RedirectStats({
  ...
  const fetchStats = async () => {
    try {
      setFetchError(null);
      const response = await api.get<RedirectStatsData>(
        "analytics/redirect-stats",
      );
      setStats(response);
      setLastUpdate(new Date());
    } catch (err: unknown) {
      const errorMessage =
        (err as Error).message || "Erro ao carregar estatísticas";
            ...
            <Grid item xs={12} sm={4}>
              <Box sx={{ textAlign: "center" }}>
                ...
                <LinearProgress
                  ...
                  value={
                    deviceTotal > 0
                      ? (stats.deviceStats.desktop / deviceTotal) * 100
                      : 0
                  }
                  sx={{ mt: 1 }}
                />
            ...
            <Grid item xs={12} sm={4}>
              <Box sx={{ textAlign: "center" }}>
                ...
                <LinearProgress
                  ...
                  value={
                    deviceTotal > 0
                      ? (stats.deviceStats.mobile / deviceTotal) * 100
                      : 0
                  }
                  color="secondary"
                  sx={{ mt: 1 }}
            ...
            <Grid item xs={12} sm={4}>
              <Box sx={{ textAlign: "center" }}>
                ...
                <LinearProgress
                  ...
                  value={
                    deviceTotal > 0
                      ? (stats.deviceStats.tablet / deviceTotal) * 100
                      : 0
                  }
                  color="success"
                  sx={{ mt: 1 }}
