...
export function usePublicAnalytics({
  ...
  const [analyticsData, setAnalyticsData] =
    useState<PublicAnalyticsData | null>(null);
  ...
  useEffect(() => {
    if (!slug) {
      setError(t("publicAnalytics.error.slugMissing"));
      setLoading(false);
      return;
    }
    ...
    const fetchData = async () => {
      try {
        ...
        if (!linkDataResult?.slug) {
          setError(t("publicAnalytics.error.invalidLinkData"));
          return;
        }

        if (!analyticsDataResult) {
          setError(t("publicAnalytics.error.invalidAnalyticsData"));
          return;
        }
        ...
        setError(
          err.response?.data?.message || t("publicAnalytics.error.loadFailed"),
        );
      } finally {
