...
export function PublicMetrics({ analyticsData }: PublicMetricsProps) {
	...
	const timeLabel = isValid ? createdDate.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' }) : '';
	...
	return (
		...
		>
			...
			>
				...
				<Typography sx={subSx}>{t('publicAnalytics.metrics.sinceCreation')}</Typography>
			</Box>
			...
			<Box sx={cardBase}>
				...
				<Box
					sx={{
						display: 'inline-flex',
						alignItems: 'center',
						background: analyticsData.is_active ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)',
						border: '1px solid',
						borderColor: analyticsData.is_active ? 'rgba(16,185,129,0.22)' : 'rgba(239,68,68,0.22)',
						borderRadius: '5px',
						px: 1.25,
						py: 0.5
					}}
				...
				<Typography sx={subSx}>{t('publicAnalytics.metrics.operational')}</Typography>
			</Box>

			<Box sx={cardBase}>
				<Typography sx={labelSx}>{t('publicAnalytics.metrics.createdAt')}</Typography>
				<Typography sx={{ fontSize: '0.9375rem', fontWeight: 700, color: 'rgba(255,255,255,0.65)', mt: 0.25 }}>
					{dateLabel}
				</Typography>
			...
			<Box sx={cardBase}>
				<Typography sx={labelSx}>{t('publicAnalytics.metrics.analyticsLabel')}</Typography>
				<Box
					sx={{
						display: 'inline-flex',
						alignItems: 'center',
						background: analyticsData.has_analytics ? 'rgba(99,102,241,0.08)' : 'rgba(255,255,255,0.04)',
						border: '1px solid',
						borderColor: analyticsData.has_analytics ? 'rgba(99,102,241,0.22)' : 'rgba(255,255,255,0.1)',
						borderRadius: '5px',
						px: 1.25,
						py: 0.5
					}}
				...
				<Typography sx={subSx}>{t('publicAnalytics.metrics.dataCollected')}</Typography>
			</Box>
		</Box>
	);
}
