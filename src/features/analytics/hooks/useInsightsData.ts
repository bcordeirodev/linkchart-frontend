...
export function useInsightsData({
     ...
     */
    const filterInsights = useCallback((insights: BusinessInsight[]): BusinessInsight[] => {
        let filtered = insights;

        // Filtrar por confiança mínima
        if (minConfidence > 0) {
            filtered = filtered.filter(insight =>
                (insight.confidence || 0.5) >= minConfidence
            );
        }
                ...
                if (Array.isArray(response.data)) {
                    ...
                    insightsData = {
                        ...
                        summary: {
                            ...
                            avg_confidence: filteredInsights.length > 0
                                ? filteredInsights.reduce((sum, i) => sum + (i.confidence || 0.5), 0) / filteredInsights.length
                                : 0
                        },
                        categories: {},
    ...
    }, [enableRealtime, refreshInterval, fetchInsightsData]);
