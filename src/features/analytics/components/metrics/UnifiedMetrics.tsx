...
import {
    ...
    MetricsLoadingIcon,
    MetricsLoadingText,
    MetricsEmptyContainer,
    MetricCardWrapper,
    CategoryIndicator
} from '../styles/Metrics.styled';
import {
    TrendingUp,
    Visibility,
    Public,
...
export function UnifiedMetrics({
    ...
    if (relevantMetrics.length === 0) {
        return (
            <MetricsLoadingContainer>
                <MetricsLoadingIcon>📊</MetricsLoadingIcon>
                <MetricsLoadingText variant="h6">
                    Carregando métricas...
                </MetricsLoadingText>
            </MetricsLoadingContainer>
        );
    ...
    // Calcular grid responsivo baseado no maxCols
    const getGridSize = () => {
        switch (maxCols) {
            case 2: return 6;  // 12/2 = 6
            case 3: return 4;  // 12/3 = 4  
            case 4: return 3;  // 12/4 = 3
            case 6: return 2;  // 12/6 = 2
            default: return 3; // Default para 4 colunas
        }
    };
        ...
        <MetricsContainer>
            {showTitle && (
                <MetricsHeader>
                    ...
                    <CategoryIndicator category={categories[0]}>
                        {categories[0]}
                    </CategoryIndicator>
                </MetricsHeader>
            )}

            <MetricsGrid container spacing={2} maxCols={maxCols}>
