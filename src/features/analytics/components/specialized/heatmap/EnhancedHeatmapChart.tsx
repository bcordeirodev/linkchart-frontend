...
export function EnhancedHeatmapChart({
	data,
	height = 600,
	title = 'Mapa de Calor Avançado'
}: EnhancedHeatmapChartProps) {
		...
		<Card sx={{ height: height }}>
			<CardContent sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
				...
				<Box sx={{ display: 'flex', gap: 2, mb: 2, flexWrap: 'wrap' }}>
					<Chip
						label={`Total: ${stats.totalClicks} cliques`}
						color="primary"
					/>
					<Chip
						label={`Países: ${stats.uniqueCountries}`}
						color="secondary"
					/>
					<Chip
						label={`Cidades: ${stats.uniqueCities}`}
						color="info"
					/>
					<Chip
