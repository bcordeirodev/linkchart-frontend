...
export function RetentionAnalysisChart({
  ...
  return (
    // `height="auto"` — a percentage height on a grid item resolves against
    // the *row*, whose height is the taller sibling's — so `height="100%"`
    // (ChartCard's default) stretched this card past its own content and
    // opened ~200px of dead space inside the border. The parent grid aligns
    // to `start`; let the card be its own height.
    <ChartCard
      height="auto"
      title={showTitle ? displayTitle : undefined}
      subtitle={showTitle ? t("insights.retention.description") : undefined}
    >
      {/* Real Metrics.
