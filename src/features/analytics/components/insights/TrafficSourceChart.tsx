...
export function TrafficSourceChart({
	...
	const channelsPieOptions = {
		...
		labels: data.channels.map((channel) => channel.channel.charAt(0).toUpperCase() + channel.channel.slice(1)),
	...
	const performanceBarOptions = {
		...
		xaxis: {
			categories: data.channels.map(
				(channel) => channel.channel.charAt(0).toUpperCase() + channel.channel.slice(1)
			),
			labels: {
				style: {
					colors: theme.palette.text.primary
