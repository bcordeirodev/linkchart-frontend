...
export function SessionDepthChart({
    ...
    const distributionBarOptions = {
        ...
        xaxis: {
            categories: data.session_distribution.map(item => `${item.session_clicks} click${item.session_clicks > 1 ? 's' : ''}`),
            labels: {
                style: {
                    colors: theme.palette.text.primary
                }
                        ...
                        <CardContent>
                            <Stack spacing={2}>
                                ...
                                <Typography variant="body1">
                                    <strong>Análise:</strong> Seus usuários clicam em média {data.avg_session_depth} vezes por sessão,
                                    com {data.power_users_percentage}% sendo power users (5+ clicks).
                                    A qualidade da sessão está classificada como <strong>{getQualityLabel(data.session_quality).toLowerCase()}</strong>.
                                </Typography>
