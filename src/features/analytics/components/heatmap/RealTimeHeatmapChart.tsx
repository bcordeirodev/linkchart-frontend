		...
		return (
			...
			>
				...
				>
					...
					{onRefresh ? (
						<Chip
							label={t('heatmap.realtimeActive')}
							color='success'
							size='small'
							sx={{ mt: 2 }}
							icon={<Zap {...ICON_MD} />}
				...
				{showControls ? (
					...
					>
						...
						{showStats && stats ? (
							...
							>
								<Chip
									label={`${stats.totalClicks.toLocaleString()} ${t('heatmap.clicks')}`}
									color='primary'
									size='small'
								/>
								<Chip
									label={`${stats.uniqueCountries} ${t('heatmap.countries')}`}
									color='secondary'
									size='small'
								/>
								<Chip
									label={`${stats.uniqueCities} ${t('heatmap.cities')}`}
									color='info'
									size='small'
								/>
								<Chip
									label={t('heatmap.avgClicks', { count: Math.round(stats.avgClicksPerLocation) })}
									color='success'
									size='small'
								/>
								{stats?.lastUpdate ? (
									<Chip
										label={`${t('heatmap.updated')} ${new Date(stats.lastUpdate).toLocaleTimeString()}`}
										variant='outlined'
										size='small'
