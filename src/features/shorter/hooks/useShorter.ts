 ...
 * `/links` (not `/links/analytics/{id}`) because a just-created link has zero clicks and the analytics page is useless without them (its view switch disables itself) — the destination carries `?created={res.id}` so `LinkListPage` highlights the new link on arrival, the same post-create flow `CreateLinkForm` uses. No `state` payload is passed: the App Router `useNavigate` shim ignores it by design.
  ...
  const handleSuccess = useCallback(
    (res: PublicLinkResponse) => {
      ...
      navTimerRef.current = setTimeout(() => {
        try {
          navigate(`/links?created=${encodeURIComponent(res.id)}`, {
            replace: true,
          });
