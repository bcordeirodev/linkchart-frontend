    ...
    const fetchRedirectData = async () => {
      ...
      try {
          ...
          userIP = await Promise.race([
            ...
            new Promise<null>((resolve) => {
              setTimeout(() => {
                // eslint-disable-next-line no-console
                console.warn(
                  "⏰ Timeout final na captura de IP (1s), prosseguindo com fallback",
                );
                resolve(null);
        ...
        if (userIP && (isValidIPv4(userIP) || isValidIPv6(userIP))) {
          ...
          // eslint-disable-next-line no-console
          console.warn(
            "⚠️ Não foi possível capturar IP real válido, backend usará fallback",
            {
              capturedIP: userIP,
              isValid: userIP
                ? isValidIPv4(userIP) || isValidIPv6(userIP)
                : false,
            },
          );
        }
