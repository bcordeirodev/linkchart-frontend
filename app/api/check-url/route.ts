...
export async function POST(request: NextRequest) {
	try {
		...
		if (!response.ok) {
			console.error('[check-url] Safe Browsing API error:', response.status, await response.text());
		...
		const threats = [...new Set(matches.map((m) => THREAT_LABELS[m.threatType] ?? m.threatType.toLowerCase()))];
