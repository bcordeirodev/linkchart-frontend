...
/**
 * Next.js adapter for the redirect page.
 *
 * The original RedirectPage reads slug via useParams (React Router), which is
 * not available in Next.js App Router. This file provides a self-contained
 * client component that accepts slug as a prop and replicates the redirect
 * logic using process.env.NEXT_PUBLIC_API_URL instead of import.meta.env.
 */
export default function RedirectPage({ params }: Props) {
	const { slug } = use(params);
	return <RedirectPageWithSlug slug={slug} />;
}

// ---------------------------------------------------------------------------
// Self-contained redirect component that accepts slug as a prop
// (avoids dependency on useParams / React Router in Next.js route)
// ---------------------------------------------------------------------------
import {
	ExternalLink,
	AlertCircle,
...
function RedirectPageWithSlug({ slug }: { slug: string }) {
		...
		try {
			...
			for (const service of ipServices) {
				try {
					...
					const response = await fetch(service, { method: 'GET', signal: controller.signal });
	...
	useEffect(() => {
		const fetchRedirectData = async () => {
			...
			try {
				...
				if (!response.ok) throw new Error(`Link não encontrado: ${response.status}`);
			...
			<>
				...
				>
					<Fade in timeout={800}>
						<Paper
							elevation={12}
							sx={{ p: 4, borderRadius: 4, textAlign: 'center', maxWidth: 450, width: '100%' }}
						>
							<Stack spacing={3} alignItems='center'>
								<AlertCircle
									{...ICON_XL}
									style={{ color: theme.palette.error.main, animation: 'pulse 2s infinite' }}
								/>
