...
- **Regressão de contraste em `UnauthorizedPage` corrigida**: descobri que a página NÃO usava `ErrorLayout` (só `NotFoundPage` usa). Sem bg do tema, herdava branco do body, deixando `text.secondary` (que no tema dark resolve para branco semitransparente) invisível. Fix cirúrgico: aplicar `bgcolor: 'background.default'` + `color: 'text.primary'` no Box wrapper externo. SignOutPage usa Paper, então não tinha o problema.

## Achados Camada 2

- **Task 21 (TabPanel) pulada**: o componente é trivial — apenas um wrapper de painel de tabs sem indicator/hover/pill/radius hardcoded. Sem hardcodes para substituir, nada a aplicar.
