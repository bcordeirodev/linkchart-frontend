...
docker compose -f deploy/docker-compose.local.yml down --remove-orphans 2>/dev/null || true
...
docker build -f deploy/Dockerfile.prod -t linkcharts-frontend:local .
...
echo "📋 Ver logs: docker compose -f deploy/docker-compose.local.yml logs -f"
echo "🛑 Parar: docker compose -f deploy/docker-compose.local.yml down"
echo "🔄 Reiniciar: docker compose -f deploy/docker-compose.local.yml restart"
echo "🧹 Limpeza completa: docker compose -f deploy/docker-compose.local.yml down --volumes --remove-orphans"
echo
